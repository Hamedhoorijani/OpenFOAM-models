...
\*---------------------------------------------------------------------------*/
#include "modifiedJohnsonJacksonFrictionalStress.H"
...
{
    defineTypeNameAndDebug(modifiedJohnsonJackson, 0);

    addToRunTimeSelectionTable
    (
        frictionalStressModel,
        modifiedJohnsonJackson,
        dictionary
    );
}
}
...
Foam::kineticTheoryModels::frictionalStressModels::modifiedJohnsonJackson::
modifiedJohnsonJackson
(
...
Foam::kineticTheoryModels::frictionalStressModels::modifiedJohnsonJackson::~modifiedJohnsonJackson()
{}
...
Foam::tmp<Foam::volScalarField>
Foam::kineticTheoryModels::frictionalStressModels::modifiedJohnsonJackson::nu
(
    const phaseModel& phase,
    const dimensionedScalar& alphaMinFriction,
    const dimensionedScalar& alphaMax,
    ...
    ) const
{
    ...
    const volScalarField& theta (phase.db().lookupObject<volScalarField>("Theta.particles"));
    forAll(D, celli)
    {
        if (alpha[celli] > alphaMinFriction.value())
        {
            nuf[celli] =
               ...
               /(
                    sqrt((1.0/3.0)*sqr(tr(D[celli])) - invariantII(D[celli]) + theta[celli]/Foam::pow(d_.value(),2))
                  + small
                );
        }
    }
...
bool Foam::kineticTheoryModels::frictionalStressModels::modifiedJohnsonJackson::read()
{
